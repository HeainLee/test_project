...
def topic_extraction(df):
    ...
    return topic_words, df

def create_dict_list_of_topics(df, topic_words):
    # Create a list of dict based on tickers and labels
    dictlist = []
    unique_list = df.dominanttopic.unique()
    unique_list = sorted(unique_list)
    print('unique_list', unique_list)
    # dominant_topic_string = ",".join(map(str, unique_list))
    # dictlist.append({'value': dominant_topic_string, 'label': dominant_topic_string})
    # return dictlist

    for dominanttopic in unique_list:
        dominant_topic_string = ",".join(str(dominanttopic))
        print("dominant_topic_string", dominant_topic_string)
        # dictlist.append({'value': dominant_topic_string, 'label': dominant_topic_string})
        dictlist.append({'value': dominant_topic_string, 'label': unique_list[int(dominant_topic_string)]})
    return dictlist